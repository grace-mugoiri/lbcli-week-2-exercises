...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '.vout[0].value * 100000000' | awk '{printf "%.0f", $0}')
...
# WRITE YOUR SOLUTION BELOW:
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '.vout[0].n')
check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '.vout[0].value * 100000000' | awk '{printf "%.0f", $0}')
...
if [ "$CHANGE_AMOUNT" -lt 0 ]; then
  echo "❌ Error: Not enough funds in selected UTXO to cover payment + fee."
  exit 1
fi
...
TX_OUTPUTS=$(printf '{"%s":%s,"%s":%s}' "$PAYMENT_ADDRESS" "$PAYMENT_BTC" "$CHANGE_ADDRESS" "$CHANGE_BTC")
...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r --arg addr "$PAYMENT_ADDRESS" '.vout[] | select(.scriptPubKey.address==$addr) | .value' | awk '{printf "%.8f", $0}')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r --arg addr "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address==$addr) | .value' | awk '{printf "%.8f", $0}')
...
if [ "$CHILD_SEND_AMOUNT" -lt 0 ]; then
  echo "❌ Error: Change output isn't large enough to cover the child transaction's fee."
  exit 1
fi
...
# STUDENT TASK: Create the outputs JSON structure
CHILD_OUTPUTS=$(printf '{"%s":%s}' "$CHILD_RECIPIENT" "$CHILD_SEND_BTC")
...
# WRITE YOUR SOLUTION BELOW:
# BIP68: for a blocks-based relative timelock, sequence == number of blocks (no disable/type flag bits set)
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$SECONDARY_TX" | jq '.vout[0].value * 100000000' | awk '{printf "%.0f", $0}')
...
if [ "$TIMELOCK_AMOUNT" -lt 0 ]; then
  echo "❌ Error: SECONDARY_TX output isn't large enough to cover the timelock fee."
  exit 1
fi
...
# STUDENT TASK: Create the outputs JSON structure
TIMELOCK_OUTPUTS=$(printf '{"%s":%s}' "$TIMELOCK_ADDRESS" "$TIMELOCK_BTC")
