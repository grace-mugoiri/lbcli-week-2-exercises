raw_tx=""

TXID=$(bitcoin-cli -regtest -rpcwallet="btrustwallet" decoderawtransaction "$raw_tx" | jq -r '.txid')

RAW_TX_09=$(bitcoin-cli -regtest -rpcwallet="btrustwallet" -named createrawtransaction \
  inputs="[{\"txid\":\"$TXID\",\"vout\":0,\"sequence\":1},{\"txid\":\"$TXID\",\"vout\":1,\"sequence\":1}]" \
  outputs='{"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP":0.2}')

printf '%s' "$RAW_TX_09"